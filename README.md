...
- This project previously ran a server-side pipeline (Python + GitHub Actions + WAQI ground-station data, committed to `data/`), and later a daily GitHub Action that logged forecast predictions to score against outcomes. Both have been retired — the site has no scheduled automation of any kind now, just the four static files fetching live from Open-Meteo on each page load. If you still have a `WAQI_TOKEN` secret configured on the repo, it's no longer used and can be removed.
