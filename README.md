...
**Live:** served by GitHub Pages from `docs/`. Installable as a PWA (works offline with the last-fetched data once visited once).

## City search

Every visit starts fresh: a welcome panel asks you to search a city or share your location — nothing is silently restored from a previous session. The search bar in the header (pre-filled with the current city once one is chosen, e.g. "Bengaluru (BLR)" — a curated short code for ~100 major world cities, omitted for anywhere not in that list) finds any place on Earth as you type, powered by the free [Open-Meteo Geocoding API](https://open-meteo.com/en/docs/geocoding-api); "Use my location" (in the welcome panel and in the search results dropdown) offers browser geolocation instead, reverse-geocoded to a real place name. The choice is reflected in the URL (`?lat=&lon=&name=…`), so a specific city's view is bookmarkable and shareable without any server-side routing — opening a link with those params boots straight to that city, skipping the welcome panel entirely, since that's explicit intent rather than an ambient "remember my last search." Every fetch — forecast, air quality, historical archive, and the four-point "around the city" AQI table — targets the selected coordinates using Open-Meteo's `timezone=auto`, so day/night theming, sunrise/sunset, and local time all resolve correctly anywhere in the world. Switching cities never shows another city's stale cached data: every `localStorage` cache key is namespaced by coordinates, and a sequence counter drops any in-flight fetch for a city you've since navigated away from.
...
- City search: Open-Meteo Geocoding API. "Use my location" reverse-geocodes your coordinates to a place name via [Nominatim](https://nominatim.org/) (OpenStreetMap's free, keyless service — this project already uses OSM data for the basemap); if that lookup fails, times out, or returns nothing (e.g. mid-ocean), it falls back to a generic "My Location" label rather than guessing.
